...
- Claude Code v2.1.178+（implicit team モデルに必要）
- Agent Teams 有効化: あなた自身の設定に `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`（v2.1.178 時点でも experimental・デフォルト無効）。`teammateMode` は任意（既定 `"in-process"`。ペイン分割表示は `"tmux"`）— プラグインがこれらを設定することはできません
