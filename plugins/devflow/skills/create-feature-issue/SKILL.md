...
Step 1-4 で起動対象とした観点のみ `TaskCreate` + `TaskUpdate` で作成（番号は下表に従う。起動しない観点は欠番でよい）。
...
`{{TEAMMATE_NAME}}` `{{INSTRUCTION}}` を置換して、Step 1-4 で起動対象とした各 Teammate（architecture-planner / existing-code-reviewer /
library-researcher / security-reviewer / ui-designer / test-planner のうち該当するもの）の
`.cache/c-f-i-t/{{TASK_SLUG}}/<role>/instruct.md` に Write してください。

特定 Teammate に追加観点が必要な場合（例: UI 設計者に「前提 Issue の要否を必ず判定すること」を追記するなど）、`## 特記事項` に記載する。architecture-planner の `## 特記事項` には「インフラ要否・IaC 設計・環境変数注入先も判定すること」を明記する。test-planner の instruct には `scope`（unit / e2e / both）を必ず指定する（UI・画面の変更を含むなら both、バックエンド/インフラ中心なら unit を目安）。

#### 3-1. Phase 1 Teammate の並列起動

instruct.md を全て書き出した後、**Step 1-4 で起動対象とした Teammate を 1 メッセージで並列起動してください**（最大 6、種別により 3〜6 体）。
...
起動対象の Teammate を同時起動したら、通知待ち受け前に `/compact` を 1 回実行してください。
...
- **Phase 1 Teammate を起動した直後に `/compact` を1回実行すること**
