...
### Phase 1: アーキテクチャ設計（リーダー + 設計レビュー〔複数 zone 時〕）
...
#### 1-3. 設計レビュー（複数 zone のときのみ）

**owner zone が 1 つ（小〜中規模）の場合はこの設計レビューをスキップ**し、1-4 へ進んでよい（リーダー自身の設計で進め、品質は Phase 3-2 の実装後レビューで担保）。owner zone が 2 つ以上（複数機能・複数 package にまたがる大規模）の場合のみ、`code-architecture-reviewer` を 1 体起動し、設計をクロスチェックしてください。コード品質・アーキテクチャ・API 契約を一体でレビューします。
...
### Phase 4: Teammate のシャットダウン

報告完了後、各 Teammate に `SendMessage` の `shutdown_request` を送信してください（チームは implicit のため解体操作は不要）。
