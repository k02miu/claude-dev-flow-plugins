...
| Verify | finding ごとに adversarial verifier を spawn し refute を試みる（自己採点バイアスを別エージェントで潰す） | 判断系 |
...
Gemini（`agy`）は workflow agent 内の Bash から起動するか、利用不可ならスキップ（副経路と同じ CLI 判定）。

### ループと Exit（構造で保証する不変条件）

- `for` ループで最大 5 iteration。`prevFindings` は変数で次 iteration に渡す（ファイル IPC 不要）。
- Exit 条件は JS で判定: (1) 全 severity が 0、(2) 前 iteration と severity 構成が同一で収束、(3) `iteration == 5`。
- **「Low も必ず対応」「件数で打ち切らない」はプロンプト祈願でなく構造で保証する**: findings schema は severity 別に全件返させ、Exit 条件 (1) は Low を含む全 severity 0 を要求する。「対応見送り推奨」の Low は schema の専用フィールドに理由付きで記録し、黙って捨てない。
...
findings は `${CLAUDE_SKILL_DIR}/references/phase-output-templates.md` の aggregated-findings 形式を schema 化したもの、最終 report は同 `criticalDecisions` を含む。
...
phase('Context')
const ctx = await agent('Run diff-summary.sh; resolve conventions; judge infra-required. Return changed files, diff summary, infra flag.',
  { schema: CTX_SCHEMA, model: 'sonnet', effort: 'low' })
...
const DIMENSIONS = [
  ...
  // ctx.infraRequired なら infra-reviewer を、codex 利用可なら codex:codex-rescue を push
]

let prev = [], exitReason = null
const history = []
for (let n = 1; n <= 5; n++) {
  phase('Review')
  ...
  const verified = await parallel(findings.map(f => () =>
    agent(`Try to refute this finding; default to refuted if uncertain: ${f.title}`, { schema: VERDICT_SCHEMA })
      .then(v => ({ ...f, verdict: v }))))
  findings = verified.filter(Boolean).filter(f => f.verdict?.isReal)
  history.push({ n, findings })

  if (findings.length === 0) { exitReason = 'all-clear'; break }
  if (sameSeverityShape(findings, prev)) { exitReason = 'converged'; break }

  phase('Fix')
  await agent(fixPrompt(findings, ctx), { agentType: 'devflow:implementer' })  // allowlist は push 非含
  prev = findings
}
if (!exitReason) exitReason = 'max-iterations'

phase('Report')
const report = await agent(reportPrompt(history, exitReason), { schema: REPORT_SCHEMA })
return report   // criticalDecisions を含む
```
