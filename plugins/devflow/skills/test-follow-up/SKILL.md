...
description: |-
  ...
  Agent Teams で test-coverage-reviewer と unit-test-planner を起動し、
...
| テスト設計 | `unit-test-planner` | `unit-test-planner` | 不足テストの設計、テストケース立案 |
...
└── {branch-name}/
    ...
    └── test-plan.md               # 追加・修正テスト設計（unit-test-planner 出力）
```
...
#### 2-3. Unit-Test-Planner の起動（不足テストが多い場合のみ）

`test-coverage-reviewer` のレポートで不足テストが多数検出された場合、
`unit-test-planner` を起動して具体的なテスト設計を行います:
...
Task tool:
  ...
  prompt: |
    あなたは unit-test-planner です。
    以下のテスト不備レポートに基づき、不足しているテストの具体的な設計を行ってください。
